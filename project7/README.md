...
- **Task 1 (2pt)**: Write a function to count the number of words in each speaking line in the Shakespeare play RDD `play.txt`. A "speaking line" is a line in which a character is speaking -- it is any line that does not begin with "ACT" or "SCENE", and does not begin or end with "*" or "/". The output will be an RDD where the key is the speaking line and the value is the number of words in the line. You may assume that splitting the line by whitespace characters will correctly separate the line into words.
...
- **Task 7 (4pt)**: [Bigrams](http://en.wikipedia.org/wiki/Bigram) are simply sequences of two consecutive words. For example, the previous sentence contains the following bigrams: "Bigrams are", "are simply", "simply sequences", "sequences of", etc. Your task is to write a bigram counting application for counting the bigrams in the `motivation`s of the Nobel Prizes (i.e., the reason they were given the Nobel Prize). The return value should be a PairRDD where the key is a bigram, and the value is its count, i.e., in how many times it appeared across all the `motivations`. Punctuation and case should be ignored. Thus "for writing", "For writing" and "for writing." are all the same bigram. In the final RDD everything should be lowercase and the following characters should be removed from the bigrams, `"`, `,`, `.`, `?`, `!`.
...
In the file `fragAndReplicate.py` (Python) or `FragmentAndReplicateJoin.java` (Java), implement the fragment-and-replicate join algorithm in the function `fragment_and_replicate` (Python) or `fragmentAndReplicateJoin` (Java) using Spark primitives. Your function must return an RDD consisting of the tuples that is the result of joining the two RDDs provided as arguments. Your function will have the following five parameters:
...
* For each cell you created above, you will have a subset of tuples from each relation (this is called a "fragment" by your textbook). Join those table fragments using a join algorithm of your choosing (nested loop join would be the easiest). You can assume that all the tuples within each fragment will easily fit in memory.
